...
/*
 * As duas variantes da mesma landing:
 *   assinar  a de venda — os botões levam ao preço e à Stripe;
 *   teste    a de campanha — os botões levam ao teste grátis, sem cartão.
 * Copy e blocos são idênticos; muda a promessa do topo e para onde cada
 * botão aponta. Os dias e tetos do teste vêm do Admin.
 */
export default async function Landing({ variante = "assinar" }: { variante?: "assinar" | "teste" }) {
  const preco = precoEmReais("prospector");
  const teste = variante === "teste";
  const cfg = await configDoTeste();
  const diasTeste = `${cfg.dias} dias`;
  ...
  return (
    <div className="pv min-h-screen overflow-x-hidden bg-[#f7f9fe] pb-20 text-[#1a1c22] md:pb-0">
      ...
      <div className="bg-[#1a1c22] px-4 py-2 text-center text-xs font-semibold text-white sm:text-sm">
        {teste
          ? `🎁 Teste grátis por ${diasTeste} · sem cartão · sem fidelidade`
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Leva ao PREÇO, não ao cadastro: ninguém cria conta antes de ver o valor.
                No teste, o valor é zero — aí sim, direto para criar a conta. */}
            <Link
              data-track={teste ? "Topo · Testar grátis" : "Topo · Ver preço"}
              href={teste ? "/assinar/teste" : "#preco"}
            ...
            >
              {teste ? `Testar grátis por ${diasTeste}` : "Ver preço e começar"}
            </Link>
          </div>
        </div>
      </header>
        ...
        <section className="relative px-5 pb-16 pt-14 sm:pt-20">
          <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1fr_1fr]">
            <div>
              ...
              <div className="mt-8">
                <Link
                  data-track={teste ? "Hero · Testar grátis" : "Hero · Prospectar no automático"}
                  href={teste ? "/assinar/teste" : "#preco"}
                ...
                >
                  {teste ? `Quero testar grátis por ${diasTeste} →` : "Quero prospectar no automático →"}
                </Link>
              </div>
              {teste ? (
                <p className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[#5f6672]">
                  <span className="font-bold text-[#188038]">🎁 Grátis por {diasTeste}</span>
                  <span>·</span>
                  <span className="font-bold text-[#1a1c22]">sem cartão</span>
                  <span>·</span>
                  <span>
                    até {cfg.empresasPorDia} empresas e {cfg.enviosPorDia} mensagens por dia
                  </span>
                </p>
              ) : (
                <p className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-[#5f6672]">
        ...
        */}
          ...
          <div className="pv-rev mx-auto mt-10 max-w-lg">
            <div className="rounded-[2rem] bg-gradient-to-br from-[#4285F4] via-[#EA4335] to-[#34A853] p-[3px] shadow-[0_36px_90px_-35px_rgba(66,133,244,.55)]">
              <div className="rounded-[calc(2rem-3px)] bg-white p-8 text-center sm:p-10">
                ...
                {teste ? (
                  <>
                    {/* Na campanha o teste é o botão principal; assinar já vem em segundo. */}
                    <Link
                    ...
                    >
                      Testar grátis por {diasTeste} →
                    </Link>
                    <p className="mt-3 text-xs text-[#5f6672]">
                      Sem cartão · até {cfg.empresasPorDia} empresas e {cfg.enviosPorDia} mensagens
                      por dia no teste · depois, R$ {preco}/mês se quiser continuar
                    </p>
                    <Link
                    ...
                    >
                      Já decidi: assinar por R$ {preco}/mês →
                    </Link>
                  </>
                ) : (
                  <>
                    ...
                    <Link
                      ...
                      className="mt-4 block rounded-full border-2 border-[#1a1c22]/15 px-8 py-3 text-base font-bold text-[#1a1c22] transition hover:border-[#25D366] hover:text-[#1f9d55]"
                    >
                      Ou teste grátis por {diasTeste}, sem cartão →
                    </Link>
                    <p className="mt-2 text-xs text-[#5f6672]">
                      No teste: até {cfg.empresasPorDia} empresas e {cfg.enviosPorDia} mensagens por
                      dia. Gostou, assina; não gostou, não paga nada.
                    </p>
                  </>
                )}
              </div>
        ...
        <section className="px-5 pb-20">
          <div className="pv-rev mx-auto max-w-lg rounded-3xl border-2 border-dashed border-[#34A853]/50 bg-[#34A853]/[0.06] p-8 text-center">
            ...
            <h2 className="mt-2 font-display text-2xl font-extrabold tracking-tight sm:text-3xl">
              {cfg.dias === 7 ? "Sete" : cfg.dias} dias grátis, sem cartão
            </h2>
            {/*
            ...
            */}
            <p className="mx-auto mt-3 max-w-md text-[15px] leading-relaxed text-[#3c4048]">
              Crie a conta, instale o Agente e prospecte de verdade por {diasTeste}:{" "}
              <b>
                até {cfg.empresasPorDia} empresas encontradas e {cfg.enviosPorDia} mensagens por dia
              </b>
              , com o funil e o remarketing inteiros. Gostou? Assina e o teto some. Não gostou? Não
              paga nada — nem pedimos cartão.
            </p>
            <Link
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-[#4285F4] to-[#34A853] px-5 py-24 text-center text-white">
          ...
          <div className="pv-rev relative mx-auto max-w-2xl">
            ...
            <Link
              data-track={teste ? "Rodapé · Teste grátis" : "Rodapé · Assinar"}
              href={teste ? "/assinar/teste" : "/assinar/prospector"}
            ...
            >
              {teste ? `Testar grátis por ${diasTeste} →` : `Começar por R$ ${preco}/mês →`}
            </Link>
            <div className="mt-3">
              <Link
                data-track={teste ? "Rodapé · Assinar" : "Rodapé · Teste grátis"}
                href={teste ? "/assinar/prospector" : "/assinar/teste"}
                className="text-sm font-bold text-white/85 underline underline-offset-4 transition hover:text-white"
              >
                {teste
                  ? `ou assinar direto por R$ ${preco}/mês`
                  : `ou testar grátis por ${diasTeste}, sem cartão`}
              </Link>
            </div>
      ...
      */}
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-black/10 bg-white/95 p-3 backdrop-blur md:hidden">
        <Link
          data-track={teste ? "Fixo · Testar grátis" : "Fixo · Começar"}
          href={teste ? "/assinar/teste" : "#preco"}
          className="block rounded-full bg-[#25D366] py-3 text-center text-sm font-bold text-white shadow-lg"
        >
          {teste ? `Testar grátis por ${diasTeste}, sem cartão →` : `Começar por R$ ${preco}/mês →`}
        </Link>
      </div>
    </div>
  );
