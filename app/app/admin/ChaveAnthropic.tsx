...
import { salvarChaveAnthropic, type ChaveIAState } from "./actions";
import { inputClass } from "@/components/painel/ui";

export default function ChaveAnthropic({ temChave }: { temChave: boolean }) {
  const [state, formAction, pending] = useActionState<ChaveIAState, FormData>(
    salvarChaveAnthropic,
    undefined,
  );
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 sm:flex-row">
      <input
        type="password"
        name="anthropic_key"
        placeholder={temChave ? "•••••••• (chave salva)" : "sk-ant-..."}
        autoComplete="off"
