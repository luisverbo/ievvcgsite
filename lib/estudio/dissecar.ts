...
function resumirAchado(a: EntradaDissecacao, i: number): string {
  const linhas = [
  ...
  linhas.push(
    a.transcricao
      ? `  TRANSCRIÇÃO: ${a.transcricao.slice(0, 6000)}`
...
export async function dissecar(
  ...
  const corpo = `TEMA pesquisado: ${tema}\n\n${achados.map(resumirAchado).join("\n\n")}`;
  const r = await chamarLLM("dissecacao", SYSTEM, corpo, 1800);

  const inicio = r.texto.indexOf("{");
  const fim = r.texto.lastIndexOf("}");
  if (inicio === -1 || fim <= inicio) {
    throw new Error("A IA não devolveu a fórmula em JSON — tente de novo.");
  }
  const bruto = JSON.parse(r.texto.slice(inicio, fim + 1)) as Partial<Formula>;
